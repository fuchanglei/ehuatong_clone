...
(function(){
    ...
    function bindBroserBtnEvt(){
        var selBtn = document.getElementById("videoSel");
            selBtn.onclick = function () {
                uploadVideoFile( this );
           
            }

    }

    function uploadVideoFile(inputFile){
        // call your api 1. get path of inputFile 2 copy real file 3.get the path of the file after copyed
       // if(!$G("videoSel").value){
       //     return;
       // }
        var path =window.external.MN_OpenFile("mp4文件|*.mp4|swf文件|*.swf");//"D:\\手机photo\\VID_20130721_132428.mp4";
        var url = $G("videoUrl");
        url.value=path;
      /* if (browser.ie) {
             url.fireEvent('onpropertychange'); 
        } else {
    ...
    */
     ...
     function insertSingle(){
        ...
        var htmlstring="<iframe width='534' height='344' style='z-index:0' "
            +" src='dialogs/video/vedio_preview.html#"
            +"videoSrc="+url
            +"&width=504"
            +"&height=324"
            +"'"
    ...
    function switchSelect( selectParentId ) {
        var selParent = $G( selectParentId ); 
        if(!selParent){
            return;
        }
        var selects = selParent.children;
